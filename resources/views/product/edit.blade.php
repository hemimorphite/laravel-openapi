...
<div class="container mt-2">
    ...
    <form action="{{ route('product.update', $product->id) }}" class="col-12 col-md-6 mb-5" method="POST" enctype="multipart/form-data">
        @method('PATCH')
        @csrf
        <div class="row">
            <div class="mb-3">
            ...
            <div id="categories">
                @if(old('categories') == null)
                    @php $isFirst = true; $index = key(array_slice($product->categories, -1, 1, true)); @endphp
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">    
                            <select class="form-select" id="category{{ $i }}" name="categories[{{ $i }}]">
                                ...
                                <option value="{{ $category->id }}" @if($cat->id == $category->id) selected @endif>{{ $category->name }}</option>
                                @endforeach
                            </select>
                            @if($isFirst)
